...
// Konfiguracja Umami
const UMAMI_URL = "https://umami.usecases.arkiv.network";
...
async function getWebsites(token) {
	...
	try {
		...
		if (response.status === 200) {
			...
			throw new Error(
				`Failed to get websites: ${response.status} - ${JSON.stringify(response.data)}`,
			);
		}
	} catch (error) {
...
async function createWebsite(token, name, domain) {
	...
	try {
		...
		if (response.status === 200 || response.status === 201) {
			...
			throw new Error(
				`Failed to create website: ${response.status} - ${JSON.stringify(response.data)}`,
			);
		}
	} catch (error) {
	...
	return websites.find((website) => {
		...
		return (
			...
			(siteDomain.startsWith("*.") &&
				normalizedDomain.endsWith(siteDomain.substring(2)))
		);
	});
}
...
async function setupAllProjects() {
	try {
		...
		for (const project of PROJECTS) {
			...
			if (website) {
				...
				try {
					...
					console.error(
						`❌ Failed to create website for ${project.name}: ${error.message}`,
					);
					results.push({
						project: project.name,
						domain: project.domain,
		...
		results.forEach((result) => {
			...
			console.log(
				`${status} | ${result.project.padEnd(12)} | ${result.domain.padEnd(20)} | ${result.websiteId || "N/A"}`,
			);

			if (result.status !== "failed") {
				console.log(
					`         | Add to .env: UMAMI_WEBSITE_ID=${result.websiteId}`,
				);
				console.log(
					`         | Tracking: <script async src="/script.js" data-website-id="${result.websiteId}"></script>`,
				);
			}
			console.log("");
		});
