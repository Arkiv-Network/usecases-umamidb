...
// Konfiguracja Umami
const UMAMI_URL =
	process.env.UMAMI_URL || "https://umami.usecases.arkiv.network";
...
async function main() {
	...
	if (!domain) {
		console.error(
			"❌ Could not detect domain. Please check your docker-compose.yml or .env file.",
		);
		console.log("Expected formats:");
		console.log(
			"- docker-compose.yml: traefik.http.routers.*.rule=Host(`domain.com`)",
		);
	...
	try {
		...
		console.log(
			`<script async src="/script.js" data-website-id="${website.id}"></script>`,
		);
	} catch (error) {
